...
def main():
	f = np.fromfile('../data/stl10/unlabeled_X.bin', dtype = np.uint8)
	image = np.array(f, dtype = np.uint8).reshape((100000, 3, 96*96))	
	preprocessed = []
	...
	np.asarray(preprocessed).astype(np.float32).tofile('whiten_gray_unlabeled_X.bin')
